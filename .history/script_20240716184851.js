    ...
    $.ajax({
      ...
      success: function(response) {
        ...
        $.each(data, function(index, value) {
          if(value.image){
            address.image = "images/default_profile.jpg";
          }
          else{
            address.image = "uploads/" + value.image;
          }
          table.row.add([
            value.id,
            value.first_name,
            value.last_name,            
            '<img src="' + address.image + '" style="width:50px;height:50px;border:2px solid gray;border-radius:8px;object-fit:cover">',
            value.email,
            value.country,
            value.gender,
